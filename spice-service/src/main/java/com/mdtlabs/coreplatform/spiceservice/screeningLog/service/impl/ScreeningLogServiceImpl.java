...
public class ScreeningLogServiceImpl implements ScreeningLogService {
	...
	public void validateScreeninigLog(ScreeningLogDTO screeningLog) {
		...
		Site site = apiInterface.getSiteById(authToken, UserSelectedTenantContextHolder.get(),
				screeningLog.getSiteId());
	...
	public PatientTracker createPatientTracker(ScreeningLogDTO screeningLog) {
		...
		String searchNationalId = screeningLog.getBioData().getNationalId();
	...
	public ScreeningResponseDTO getScreeningDetails(RequestDTO screeningRequestDto) {
		...
		ScreeningResponseDTO screeningResponse = modelMapper.map(screeningLog, new TypeToken<ScreeningResponseDTO>() {
		}.getType());
		...
		if (!Objects.isNull(latestBpLog)) {
			...
			screeningResponse.setRegularSmoker(latestBpLog.isRegularSmoker());
		...
		if (!Objects.isNull(latestGlucoseLog)) {
			...
			screeningResponse.getGlucoseLog().setGlucoseId(latestGlucoseLog.getId());
		}
