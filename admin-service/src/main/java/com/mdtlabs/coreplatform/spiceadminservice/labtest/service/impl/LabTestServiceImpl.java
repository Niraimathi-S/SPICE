...
/**
 * <p>
 * This service class contain all the business logic for lab test module and perform
...
public class LabTestServiceImpl implements LabTestService {
	...
	public LabTest addLabTest(LabTest labTest) {
		if (Objects.isNull(labTest)) {
			...
			LabTest existingLabtest = labTestRepository
				.findByCountryIdAndNameAndIsDeleted(labTest.getCountryId(), labTest.getName(), false);
	...
	public List<Map> searchLabTests(RequestDTO requestDto) {
		...
		List<LabTest> labTests = labTestRepository.searchLabTests(
			formattedSearchTerm, requestDto.getCountryId(), requestDto.getIsActive());
		return labTests.stream().map(
			labTest -> Map.of("id", labTest.getId(), "name", 
			labTest.getName(), "country", labTest.getCountryId())).collect(Collectors.toList());
	}
	...
	public boolean removeLabTest(RequestDTO requestDto, boolean status) {
		LabTest labTest = labTestRepository.findByIdAndIsDeleted(
			requestDto.getId(), Constants.BOOLEAN_FALSE);
		if (Objects.isNull(labTest)) {
	...
	public LabTest updateLabTest(LabTest labTest) {
		...
		mapper.createTypeMap(LabTest.class, LabTest.class)
			.addMappings(map -> map.skip(LabTest::setLabTestResults));
	...
	private void validateLabTestResultsAndUpdate(LabTest labTest, LabTest existingLabTest) {
		...
		Map<Long, String> oldResults = existingLabTest.getLabTestResults().stream()
		...
		for (LabTestResult result : labTest.getLabTestResults()) {
			if (Objects.isNull(result.getId())) {
				...
				if (oldResults.values().contains(result.getName())
					&& oldResults.get(result.getId()) != result.getName()) {
		...
		List<String> names = newLabTestResults.stream().map(name -> name.getName())
			.collect(Collectors.toList());
		if (updatedLabTestResults.values().stream().anyMatch(data -> names.contains(data.getName()))) {
	...
	private LabTest validateLabTestRequest(LabTest labTest) {
		...
		LabTest existingLabTest = labTestRepository.findByIdAndIsDeleted(
			labTest.getId(), Constants.BOOLEAN_FALSE);
		if (Objects.isNull(existingLabTest)) {
			// labtest doesn't exist
			throw new SpiceValidation();
		}
		if (!Objects.isNull(labTest.getName()) && !existingLabTest.getName()
			.equals(labTest.getName())) {
			// Lab Test name should not be changed.
			throw new SpiceValidation();
		}
		if (!Objects.isNull(labTest.getCountryId())
			&& existingLabTest.getCountryId() != existingLabTest.getCountryId()) {
	...
	public LabTest getLabTestById(RequestDTO requestDto) {
		...
		if (Objects.isNull(labTest)) {
			...
			labTest.setLabTestResults(labTest.getLabTestResults().stream()
				.filter(labTestResult -> !labTestResult.isDeleted())
	...
	public Boolean validateLabTest(LabTest labTest) {
		LabTest labTestCountryDetail = labTestRepository
			.findByCountryIdAndNameAndIsDeleted(labTest.getCountryId(), labTest.getName(), false);
