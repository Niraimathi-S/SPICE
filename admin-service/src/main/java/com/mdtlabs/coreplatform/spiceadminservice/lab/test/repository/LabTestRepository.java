...
public interface LabTestRepository extends JpaRepository<LabTest, Long> {
    ...
    public static final String GET_ALL_LABTESTS = "select labtest from LabTest as labtest where "
        ...
        + "lower(labtest.name) LIKE CONCAT('%',lower(:searchTerm),'%'))";

    public static final String GET_LABTEST_BY_MANDATORY_FIELDS = "select labtest from LabTest as "
    	+ "labtest where labtest.countryId=:country AND "
    	+ "labtest.name=:name AND labtest.isDeleted=false order by labtest.updatedAt desc";

	public static final String GET_LABTEST_BY_NAME = "select labtest from LabTest as labtest"
		+ " where lower(labtest.name) LIKE CONCAT('%',lower(:searchTerm),'%')"
		+ " AND labtest.countryId=:countryId AND labtest.isDeleted=false and "
		+ "(:isActive is null or labtest.isActive=:isActive)"
    ...
    public List<LabTest> searchLabTests(@Param("searchTerm") String searchTerm, 
    	@Param("countryId") Long countryId, @Param("isActive") Boolean isActive);
