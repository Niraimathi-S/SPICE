...
public interface MedicationRepository extends JpaRepository<Medication, Long> {
	...
	public static final String UPDATE_MEDICATION_STATUS_BY_ID = "update from Medication as medication set "
		...
		+ " medication.isDeleted=false AND medication.tenantId =:tenantId";

	public static final String GET_MEDICATION_BY_MEDICATION_NAME = "select medication from Medication "
		+ "as medication where lower(medication.medicationName) LIKE CONCAT('%',lower(:searchTerm),'%') "
		+ " AND medication.countryId=:countryId AND medication.isDeleted=false"
		+ " ORDER BY medication.medicationName ASC,medication.updatedAt DESC";

	public static final String GET_MEDICATION_BY_MANDATORY_FIELDS = "select medication from Medication "
	...
	public static final String GET_OTHER_MEDICATION = "select * from Medication as medication "
		+ "where medication.country_id = :countryId AND lower(medication.brand_name) "
		+ "LIKE CONCAT(lower(:brandName),'%') AND lower(medication.name) LIKE "
		+ "CONCAT(lower(:medicationName),'%') AND lower(medication.classification_name) "
		+ "LIKE CONCAT(lower(:classificationName),'%') AND lower(medication.dosage_form_name) "
		+ "LIKE CONCAT(:dosageFormName,'%') limit 1";
