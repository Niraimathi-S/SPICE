...
public interface RoleRepository extends JpaRepository<Role, Long>, PagingAndSortingRepository<Role, Long> {
	...
	public static final String GET_ALL_ROLES = "select role from Role as role"
		+ " where role.isActive =:status ";
	public static final String UPDATE_ROLE_STATUS_BY_ID = "update Role as role "
		+ "set role.isActive =:status where role.id =:roleId";
	public static final String GET_ROLE_BY_ID = "select role from Role as role "
	...
	public int updateRoleStatusById(@Param(FieldConstants.STATUS) Boolean status, 
		@Param(Constants.ROLE_ID_PARAM) long roleId);
